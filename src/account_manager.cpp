...
#include <iostream>
#include <vector>
void User::changepass(char *new_pass) { memcpy(password, new_pass, 31); }
void AccountManager::login(char *userid, char *pas) {
  ...
  if (memcmp(users.password, pas, 31) != 0) {
    std::cout << "Invalid\n";
    return;
  }
  ubook uboo;
...
void AccountManager::login(char *userid) {
  ...
  int curpri = 0;
  if (!accounts.empty())
    curpri = accounts.back().user.privilege;
  if (curpri <= users.privilege) {
    std::cout << "Invalid\n";
    return;
  }
  ubook uboo;
...
void AccountManager::changeinf(char *userid, char *old_pas, char *new_pas) {
  ...
  User users = found.back();
  if (memcmp(old_pas, users.password, 31) != 0) {
    std::cout << "Invalid\n";
    return;
  }
...
void AccountManager::deleteuser(char *userid) {
  ...
  for (auto it = accounts.begin(); it != accounts.end(); it++) {
    if (memcmp((*it).user.id, userid, 31) == 0) {
      std::cout << "Invalid\n";
      return;
    }
  }
