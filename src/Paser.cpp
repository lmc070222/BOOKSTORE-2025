...
public:
  void parseCommand(const std::string &command) {
    ...
    std::regex registerregex(
        R"(register ([A-Za-z0-9_]{1,30}) ([A-Za-z0-9_]{1,30}) ([\x21-\x7E]{1,30}))");
    std::regex passwdregex2(
        R"(passwd ([A-Za-z0-9_]{1,30}) ([A-Za-z0-9_]{1,30}))");
    std::regex passwdregex3(
        R"(passwd ([A-Za-z0-9_]{1,30}) ([A-Za-z0-9_]{1,30}) ([A-Za-z0-9_]{1,30}))");
    ...
    if (!accountManager.accounts.empty())
      curpri = accountManager.accounts.back().user.privilege;
    if (std::regex_match(cmd, match, suregex)) {
      ...
      if (Quantity <= 0 || Quantity > 2147483647LL) {
        std::cout << "Invalid\n";
        return;
      }
      Book book;
      ...
      if (found.back().remain_num < Quantity) {
        std::cout << "Invalid\n";
        return;
      }
      Transaction txn;
      ...
      bool hasISBN = false, hasName = false, hasAuthor = false,
           hasKeyword = false, hasPrice = false;
      ...
      if (Quantity <= 0 || Quantity > 2147483647LL || TotalCost <= 0) {
        std::cout << "Invalid\n";
        return;
      }
