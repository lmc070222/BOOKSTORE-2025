...
void BookManager::modify(Book new_inf, AccountManager &account_manager,
                         int flag) {
  Book tmp;
  if ((flag == 1 and
       memcmp(new_inf.ISBN, account_manager.accounts.back().book.ISBN, 21) ==
           0) or
      account_manager.accounts.back().book.if_same(tmp)) {
    std::cout << "Invalid\n";
    return;
  }
...
void BookManager::import(long long Quantity, double TotalCost,
                         AccountManager &account_manager) {
  Book tmp;
  if (memcmp(tmp.ISBN, account_manager.accounts.back().book.ISBN, 21) == 0) {
    std::cout << "Invalid\n";
    return;
  }
  tmp = account_manager.accounts.back().book;
...
void BookManager::show(std::string type, char *inf) {
  ...
  if (type == "ISBN") {
    ...
    for (size_t i = 0; i < boo.size(); i++) {
      std::cout << boo[i].ISBN << '\t' << boo[i].bookname << '\t'
                << boo[i].bookauther << '\t' << boo[i].keyword << '\t'
                << std::fixed << std::setprecision(2) << boo[i].price << '\t'
                << boo[i].remain_num << '\n';
    }
  } else if (type == "name") {
    Bookbookname bookname_;
    ...
    for (size_t i = 0; i < allbooks.size(); i++) {
      std::cout << allbooks[i].ISBN << '\t' << allbooks[i].bookname << '\t'
                << allbooks[i].bookauther << '\t' << allbooks[i].keyword << '\t'
                << std::fixed << std::setprecision(2) << allbooks[i].price
                << '\t' << allbooks[i].remain_num << '\n';
    }
  } else if (type == "auther") {
